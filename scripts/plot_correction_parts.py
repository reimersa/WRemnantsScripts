...
def main():
    ...
    # filename_fullcorr = ("/work/submit/areimers/wmass/WRemnants/wremnants-data/data/TheoryCorrections/scetlib_dyturbo_LatticeNP_FineBins_CT18Z_N3p0LL_N2L0CorrZ.pkl.lz4", "Full N$^{3{+}0}$LL+NNLO")
    ...
    filename_fullcorr = ("/work/submit/areimers/wmass/WRemnants/wremnants-data/data/TheoryCorrections/scetlib_dyturbo_LatticeNP_FineBins_CT18Z_N3p0LL_N2L0_pdfas_CorrZ.pkl.lz4", "Full N$^{3{+}0}$LL+NNLO")
    ...
    # variations = [({"vars" :  "pdfCT18ZNNLO_as_0118"}, "($\\alpha_{\\mathrm{s}} = 0.118\\pm0.002$)"), ({"vars" :  "pdfCT18ZNNLO_as_0116"}, "($\\alpha_{\\mathrm{s}} = 0.116$)"), ({"vars" :  "pdfCT18ZNNLO_as_0120"}, "($\\alpha_{\\mathrm{s}} = 0.120$)")]
    # vartag = "asvars"
    variations = [({"vars" :  "pdfCT18ZNNLO_as_0118", "Q": 1}, "($\\alpha_{\\mathrm{s}} = 0.118\\pm0.002$)"), ({"vars" :  "pdfCT18ZNNLO_as_0116", "Q": 1}, "($\\alpha_{\\mathrm{s}} = 0.116$)"), ({"vars" :  "pdfCT18ZNNLO_as_0120", "Q": 1}, "($\\alpha_{\\mathrm{s}} = 0.120$)")]
    vartag = "asvars"
    # variations = [({"vars" :  0, "Q": 0}, "($\\mu_{r} = 1$)"), ({"vars" :  "kappaFO0.5-kappaf2.", "Q": 0}, "($\\mu_{r} = 0.5$)"), ({"vars" :  "kappaFO2.-kappaf0.5", "Q": 0}, "($\\mu_{r} = 2$)")]
    # vartag = "murvars"

    # variations = [({"vars" :  0, "Q": 1}, "nominal")]
    # vartag = "nominal"
    ...
    vars_resum      = [hist_resum[v[0]].project("qT") for v in variations]
    vars_fo         = [hist_fo[v[0]].project("qT") for v in variations]
    vars_fullcorr   = [hist_fullcorr[v[0]].project("qT") for v in variations]
    vars_fosing_neg = [hist_fosing[v[0]].copy() for v in variations]
    ...
    labels = [f"{filename_fullcorr[1]} {variations[0][1]}"] + [""]*(len(variations)-1) + [f"{filename_resum[1]} {variations[0][1]}"] + [""]*(len(variations)-1) + [f"{filename_fosing[1]} {variations[0][1]}"] + [""]*(len(variations)-1) + [f"{filename_fo[1]} {variations[0][1]}"] + [""]*(len(variations)-1)
